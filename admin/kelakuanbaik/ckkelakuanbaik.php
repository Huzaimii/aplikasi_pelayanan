...
$pdf->Setfont("Arial", "B", "13");
	$pdf->cell(20,0.7,'SURAT KETERANGAN KELAKUAN BAIK',0,1,'C');

	$pdf->Setfont("Arial", "", "10");
	$pdf->cell(20,0.27,'NOMOR :' .$data['id_kelakuan_baik']. '/KB/2023',0,1,'C');


	$pdf->ln(0.5); //fungsi ln sama seperti <br>


	$pdf->Cell(5, 0.4, 'Yang bertanda tangan dibawah ini Pembakal Desa Tanah Abang Kecamatan Mataraman menerangkan bahwa:', 3, 1, '');
...
$pdf->Cell(1, 0, '', 0, 0, '');
	...
	$pdf->Cell(1, 0, '', 0, 0, '');
 $pdf->Cell(5, 1, 'Status Perkawinan', 0, 0, '');
	$pdf->Cell(2, 1, ': '.$data['status_kawin'].'', 0, 1, '');
	$pdf->Cell(1, 0, '', 0, 0, '');
	$pdf->Cell(5, 0, 'Alamat', 0, 0, '');
	$pdf->Cell(2, 0, ': '.$data['alamat_p'].'', 0, 1, '');
	$pdf->Cell(1, 0, '', 0, 0, '');
	...
	$pdf->Cell(5, 0.4, 'Adalah benar warga/penduduk Desa Tanah Abang Kecamatan Mataraman Kabupaten Banjar,', 3, 1, '');
	$pdf->Cell(5, 0.4, 'berdasarkan data serta pengetahuan kami bahwa nama tersebut :', 3, 1, '');
	$pdf->Cell(5, 1, '1.Berkelakuan '.$data['berkelakuan_baik']. '.', 3, 1, '');
	$pdf->Cell(5, 0, '2.'.$data['pecandu_narkoba'].' termasuk pecandu narkoba dan minuman keras.', 3, 1, '');

	$pdf->Cell(5, 1, '3.'.$data['terlibat_pidana']. ' pernah terlibat tindak pidana maupun perdata.', 3, 1, '');

	$pdf->Cell(5, 0.4, 'Surat keterangan ini dibuat dan diberikan untuk mendapatkan rekomendasi dari Kapolsek Mataraman,', 3, 1, '');
	$pdf->Cell(5, 0.4, 'untuk keperluan '.$data['keperluan']. '.', 3, 1, '');
